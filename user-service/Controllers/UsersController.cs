...
namespace user_service.Controllers
{
	...
	public class UsersController : ControllerBase
	{
		...
		[HttpPut("birthday")]
		public async Task<ActionResult<object>> UpdateBirthday([FromBody] UpdateBirthdayDTO birthdayDto)
		{
			try
			{
				var userId = GetCurrentUserId() ?? string.Empty;
				await _userService.UpdateBirthdayAsync(userId, birthdayDto.Birthday);
			...
			catch (Exception)
			{
				return StatusCode(StatusCodes.Status500InternalServerError, 
					new { message = "An error occurred while updating birthday." });
			}
		}
	}
}
